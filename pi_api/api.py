...
def restart_shepherd():
    """Restart shepherd so a freshly deployed shepherd.py runs (Deploy calls this). Only when
    shepherd is running: a Pi without it, or with the Monitor toggle off, is left alone. Waits
    for the restart, so the new shepherd is up on return."""
    active = subprocess.run(["systemctl", "is-active", "shepherd"],
                            capture_output=True, text=True, timeout=10)
