"""
devices/photodiode_filtered.py  —  DEPRECATED (stashed)

Original photodiode sync input that did the STEADY (pigpio glitch) + HOLD-OFF (debounce)
filtering ON THE PI. Superseded by devices/photodiode.py, which reads a clean, pre-filtered
TTL from a Teensy (the Teensy now runs those two filters — see teensy/photodiode_sync.ino).

Kept only as a stash in case the Teensy path is abandoned. It is NOT imported anywhere, so
it does nothing until re-enabled. To go back to on-Pi filtering:
...
@register_device
class PhotodiodeFiltered(Device):
    info = DeviceInfo("photodiode_filtered", "Photodiode (filtered — deprecated)",
                      IOType.GPIO_IN, ["pigpio"])
