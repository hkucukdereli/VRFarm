/*
 * photodiode_sync_v2_1.ino  —  Teensy 4.x photodiode sync-pulse detector   (VERSION v2_1)
 *
 * v2_1 adds an ALWAYS-ON serial stream of the photodiode voltage V — one bare float per line at
 * 100 Hz — so the LEADER Pi can monitor signal quality and validate the display->diode sync path
 * at init. Connect the Teensy USB to the LEADER Pi (it runs the photodiode device); the TTL pulse
 * wire (OUT_PIN -> leader GPIO) is unchanged, so the Teensy reaches the leader two ways: USB (V
 * stream, new) + the pulse wire (timing). The stream is independent of DEBUG, which stays a
 * separate Serial-Plotter tuning tool (off by default).
 *
...
// ── Always-on V stream (v2_1) ────────────────────────────────────────────────
// Independent of DEBUG: when DEBUG is OFF, print the BARE photodiode voltage — one float per line,
// nothing else — at STREAM_PERIOD_US so the leader Pi parses it with float(line) for V-range
// monitoring and the init sync check. Detection/output are unaffected. (When DEBUG is ON the
// 4-trace plotter stream is emitted instead, whose first field is also V.)
const uint32_t STREAM_PERIOD_US = 10000;   // 10 ms = 100 Hz
...
uint32_t lastStreamUs = 0;       // v2_1: gates the always-on bare-V serial stream
...
#if !DEBUG
  // ── v2_1 always-on stream: bare photodiode voltage @ 100 Hz for the leader Pi. One float per
  //    line, nothing else. (Suppressed when DEBUG is on — the 4-trace block below carries V then.)
  if (now - lastStreamUs >= STREAM_PERIOD_US) {
    lastStreamUs = now;
    Serial.println(countsToVolts(counts) * DIVIDER, 3);
  }
#endif
