...
generic machinery — the four-tab controller, session engine, data saving, livestreams,
deploy/install, data sync — so new projects fork from here and add their own devices.
...
into. Devices are pluggable and fully generic: this branch ships **zero** concrete devices.
...
```bash
python tools/mock_pi.py                 # fake pi_api :5080 + fake leader (rig demo)
...
- Keep Pi-deployed code compatible with the OLDEST Pi python in the fleet (bullseye = 3.9:
  no `match`, no runtime `X | Y` unions). Annotations are fine behind
