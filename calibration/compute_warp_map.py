...
def build_projector(geo):
    ...
    Returns a dict with:
      pos:        projector lens position (x, y, z) in inches
      ...
      img_w:      projected image width in inches
      img_h:      projected image height in inches
      res:        (width, height) in pixels
    """
    p = geo['projector']
    res_w, res_h = p['resolution']
    throw_dist = p['throw_distance_cm'] / 2.54  # → inches
    throw_ratio = p['throw_ratio']
    ax_el = np.radians(p['optical_axis_elevation_deg'])
    lat_offset = p['lateral_offset_inches']

    img_w = throw_dist / throw_ratio
    img_h = img_w * (res_h / res_w)
    ...
    # Projector optical axis: from projector toward screen center
    # Projector is behind and above the screen, along the optical axis direction
    # ax_el is the elevation of the beam (looking from projector toward screen)
    # Forward vector (from projector toward screen): downward and forward
    fwd = np.array([
    ...
    # Projector position: screen_center - throw_dist * forward
...
def screen_point_to_pixel(pt, proj):
    ...
    # Image plane: x_img in [-img_w/2, +img_w/2] → px in [0, res_w]
    res_w, res_h = proj['res']
    px = (x_img / proj['img_w'] + 0.5) * res_w
    py = (1.0 - (y_img / proj['img_h'] + 0.5)) * res_h   # flip y (image coords)
...
def compute_inverse_map(geo, proj):
    ...
    # Pixel → image plane position (inches relative to image center)
    x_img = (PX / res_w - 0.5) * proj['img_w']
    y_img = (0.5 - PY / res_h) * proj['img_h']   # flip y
...
def main(validate=False):
    ...
    print(f"  Projector position: ({proj['pos'][0]:.2f}, {proj['pos'][1]:.2f}, {proj['pos'][2]:.2f}) inches")
    print(f"  Image size: {proj['img_w']:.2f}\" × {proj['img_h']:.2f}\"")
